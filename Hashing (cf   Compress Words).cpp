...
string a,b;
vector<PLL>ha(1000006);
vector<PLL>hb;

vector<PLL> hashList(int n)
{
    ...
    for (int i=1; i<=n; i++)
        ans[i] = (ans[i-1] * base + b[i-1])%M;
    return ans;
}

void hashLista(string &s,LL l)
{
    int n = s.size();
    for (int i=l+1; i<=n; i++)
    {
        ha[i] = (ha[i-1] * base + s[i-1])%M;
    }
...
LL  ok(int l)
{
    ...
    for(int j=l; j>=1; j--)
    {
        PLL  d1=substringHash(hb,1,j);
        PLL  d2=substringHash(ha,n-j+1,n);
        if(d1==d2)return j;
    }
...
int main()
{
    ...
    while(t--)
    {
        ...
        else hb=hashList(szb);
        LL r=min(szb,sza);
        l=ok(r);
        start=sza;
        for(int i=l; i<szb; i++)a+=b[i];
