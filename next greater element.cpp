...
stack<ll>st;
  ...
  for(int i=0;i<n;i++)
  {
    ...
    //cout<<a[i]<<" "<<mx_left[i]<<" "<<mx_right[i]<<endl;

    if(mx_left[i]!=-1 and ma[abs(mx_left[i]-a[i])]==0)
    {
        ans++;
       ma[abs(a[i]-mx_left[i])]=1;
   }

     if( mx_right[i]!=-1 and ma[abs(mx_right[i]-a[i])]==0)
    {
        ans++;
       ma[abs(a[i]-mx_right[i])]=1;
   }
}
