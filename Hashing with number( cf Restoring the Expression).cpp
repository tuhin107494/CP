...
bool ok(int a,int b,int c,int n)
{
    ...
    if(aa[0]=='0' and a>1)return 0;
    if(aa[a]=='0' and b>1)return 0;
    if(aa[a+b]=='0' and c>1)return 0;
    PLL pa=substringHash(ha,1,a);
    PLL pb=substringHash(ha,a+1,a+b);
    PLL pc=substringHash(ha,n-c+1,n);
...
int main()
{
    ...
    for(int i=1; i<=n; i++)
    {
        ...
        if((max(a,b)==c or max(a,b)==c-1) and min(a,b)>0)
        {
            if(ok(a,b,c,n))
            {
                cout<<aa.substr(0,a)<<"+"<<aa.substr(a,b)<<"="<<aa.substr(a+b,c)<<endl;
                break;
            }
            swap(a,b);
            if(ok(a,b,c,n))
