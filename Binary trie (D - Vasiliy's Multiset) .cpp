...
///====================================StartHere=================////////////
int to[mx7][2],to_node=1;
map<int,map<int,int> >cnt;
int node[mx7];
void trie_add_string(bitset<30> s,int num)
{
    int cur=1;
    cnt[cur][num]++;
    node[cur]++;
...
void trie_delete_string(bitset<30> s,int num)
{
    int cur=1;
    if(cnt[cur][num]>0){node[cur]--;cnt[cur][num]--;}
    else return;
...
int trie_quary(bitset<30> s)
{
    ...
    {
        int ch=s[i];
        if(to[cur][ch^1] and node[to[cur][ch^1]])
        {
            cur=to[cur][ch^1];
            ans+=d;
        }
...
void input()
{
    ...
    if(a=="+")trie_add_string(ma,n);
    else if(a=="-")trie_delete_string(ma,n);
    else cout<<trie_quary(ma)<<endl;
