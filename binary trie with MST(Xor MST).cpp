...
void trie_add_string(int  num,int id)
{
    bitset<31>s(num);
    //debug2(num,id);
    int cur=1;
...
void trie_delete_string(int num,int id)
{
    bitset<31>s(num);
    int cur=1;
    node[cur]--;
    ...
    auto it=track[cur].find(id);
    track[cur].erase(it);

}
int trie_quary(int num )
{
    ...
    {
        int ch=s[i];
        if(to[cur][ch] and node[to[cur][ch]]) cur=to[cur][ch];
        else  cur=to[cur][ch^1];

    }
    return *track[cur].begin();
}
...
void input()
{
    ...
    while(com>1)
    {
        ...
        for(int i=1; i<=cs; i++)
        {
            ...
            for(int u=0; u<v[i].size(); u++) trie_delete_string(a[v[i][u]],v[i][u]);
            int mn=INT_MAX,vv=-1;
            for(int u=0; u<v[i].size(); u++)
            {
                int  in=trie_quary(a[v[i][u]]);
                if((a[in]^a[v[i][u]])<mn)
                {
                    mn=a[in]^a[v[i][u]];
                    vv=in;
            ...
            for(int u=0; u<v[i].size(); u++) trie_add_string(a[v[i][u]],v[i][u]);
            vv=find(vv);
            int parv=find(vv);
            int pari=find(i);

            if(parv==pari)continue;

            com--;

            if(v[parv].size()>v[pari].size())
            {
                par[pari]=parv;
                for(auto  u:v[pari])v[parv].pb(u);
                v[pari].clear();
            }
            else
            {
                par[parv]=pari;
                for(auto  u:v[parv])v[pari].pb(u);
                v[parv].clear();
            }
