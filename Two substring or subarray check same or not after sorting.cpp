...
const PLL M=mp(1088888881, 1111211111);   ///Should be large primes
...
vector<PLL> hashList(vector<LL> &s,int id)
{
    int n = s.size();
    vector<PLL> ans(n+1);
    ans[0] = mp(0,0);
...
{
    int len = (r-l+1);
    return ((hashlist[r] -hashlist[l-1])%M+M)%M;
}
...
int main()
{
    ...
    vector<PLL> ha=hashList(v,1);
      vector<PLL> hb=hashList(v,2);

    while(m--)
    {
      LL x1,y1,x2,y2;
        cin>>x1>>y1>>x2>>y2;
        // cout<<substringHash(ha,x1,y1)<<" "<<substringHash(ha,x2,y2)<<endl;
        if(substringHash(ha,x1,y1)==substringHash(ha,x2,y2) and substringHash(hb,x1,y1)==substringHash(hb,x2,y2) )cout<<"Yes"<<endl;
        else cout<<"No"<<endl;
    }
